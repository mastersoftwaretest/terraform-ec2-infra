resource "aws_instance" "public" {
  ...
  key_name                    = "jq-key-pair" #Change to your keyname, e.g. jazeel-key-pair
  vpc_security_group_ids = [aws_security_group.allow_ssh.id]
 
  tags = {
    Name = "jq-ec2"    #Prefix your own name, e.g. jazeel-ec2
  }
}

resource "aws_security_group" "allow_ssh" {
  name        = "jq-terraform-security-group" #Security group name, e.g. jazeel-terraform-security-group
